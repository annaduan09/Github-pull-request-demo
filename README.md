# Urban Tech Club Github Conflict Demo 😈
Oct 1, 2024
Anna Duan


![Merge conflict vibes](https://github.com/annaduan09/Github-conflict-demo/blob/30794f50c655c91d7c05b2bed9a6432eb5646ee5/merge%20conflict.png)

In this demo, participants will learn the basics of using Github repositories for version control, and practice the following functions:  
- **Fork**  
- **Clone**  
- **Commit**  
- **Push**  

They will also get practice collaborating on repositories, including best practices for working simultaneously and resolving conflicts. Data for this activity comes from [OpenDataPhilly.](https://metadata.phila.gov/#home/datasetdetails/5dc1aeb93741fa001504b10b/representationdetails/5dc1aeb93741fa001504b10f/)

### Part 0: Setup 
1. Split up into 3 groups
2. From each group, nominate one person to type on their laptop

### Part 1: Using Github repositories  
#### Group 1:  
1. **Fork** this repo. 
2. Invite Group 2 and Group 3's scribe to collaborate. 

### Part 2: Creating branches  
#### Group 1: 
1.  Open a branch, name it "adding-map-labels"
2. Add a title and date to the ggplot2 map
3. Go to Github Desktop to **commit** and **push** your changes when satisfied.

#### Group 2: 
1. Open a branch from Group 1's repo, name it "adjust-theme-and-colors". 
2. Change the map's theme to theme_minimal() and modify the colors to your liking. 
3. Go to Github Desktop to **commit** and **push** your changes when satisfied.

#### Group 3:
1. Open a branch from Group 1's repo, name it "add-water-features".
2. Add the water features into the map's bottom-most layer using the water_features object defined in the code; adjust colors as needed.
3. Go to Github Desktop to **commit** and **push** your changes when satisfied.

### Part 3: Pull requests and merging  
1. Everybody goes to Github web and opens a pull request. Add a brief description of what you did.
2. Review time: group 1 reviews 2, 2 reviews 3, 3 reviews 1. Add a nice comment for your friend's work!  
3. Groups merge your own changes if no conflicts arise.
4. Admire the map we made together!